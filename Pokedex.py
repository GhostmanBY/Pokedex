kanto_pokedex = {
    ...
    "Mew": 151
} #pokedex de los 151 pokemones de Kanto

def serch_pokemon_name(pokemon): #funcion para buscar un pokemon por su nombre
    if pokemon in kanto_pokedex: #verifica si existe un pokemon con el nombre ingresado en el diccionario de la pokedex de kanto
        return pokemon #si existe retorna el nombre
    else: 
        return None #en caso falso retorna None

def serch_pokemon_num(num_pokemon): #funcion para buscar un pokemon por su numero en la pokedex
    for nombre, numero in kanto_pokedex.items(): #va iterando dentro del diccionario de kanto
        if num_pokemon == numero: #evalua en cada itrancion si el numero del nopkemon conincide con el de la pocicion actual
            return nombre #Si es asi retorna su nombre corespondiente
        elif numero > 151:#caso falso, este caso se ejecuta cuando numero es mayor al limite de la pokedex
            return None #retornando un None